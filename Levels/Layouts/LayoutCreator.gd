...
func get_layout(floor: int):
	thread = Thread.new()
	thread.start(_thread_get_layout.bind(floor))
...
func _thread_get_layout(floor: int):
	var root = _get_line_layout(floor)
	_level_layout = root

func _get_line_layout(floor: int) -> LayoutNode:
	var root = LayoutNode.new(0, LayoutNode.RoomType.SPAWN)
	
	# make main boss line
	var cur_node = root
	for i in LINE_BOSS_TARGET_DEPTH:
		for n in get_room_length(floor):
	...
	make_path(LINE_CHEST_2_TARGET_DEPTH, root, floor, get_chest_node(LINE_CHEST_2_TARGET_DEPTH))
	make_path(LINE_CHEST_1_TARGET_DEPTH, root, floor, get_chest_node(LINE_CHEST_1_TARGET_DEPTH))
	# make shop path
	var shop_depth = get_line_shop_depth()
	make_path(shop_depth, root, floor, get_shop_node(shop_depth))
	
	root.debug_print()
	return root
	
func make_path(target_depth: int, root: LayoutNode, floor: int, leaf: LayoutNode):
	# make path to chest1
	var cur_node = root
	...
	for i in target_depth:
		...
		if randf_range(0, target_depth-1) < cur_node.depth:
			...
			for n in range(cur_node.depth, target_depth):
				for m in get_room_length(floor):
...
func get_room_length(floor: int) -> int:
	match floor:
		1:
			return 1
		2:
